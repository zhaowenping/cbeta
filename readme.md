...
2. 汉字处理区分为原版和阅读版。
   ...
   2. 原版: 使用UNICODE 14.0版本; 在此基础上尽力显示经文原貌, 使用藏经原有字形, 补全cbeta缺失的生僻字. 恢复部分异体字在藏经中的原貌
   3. 去除了中日韩兼容字符区（F900 ~ FAFF）的汉字，只保留了(﨑、﨟)两个字; 去除了兼容汉字区的汉字,只保留了(你 U+2F804)一个字(未来使用ivd方式去除)，吆(U+2F83B)字完全替换为䶸(U+4DB8)
   4. 规范化汉字使用, 规范化标点使用.
...
## 目标
1. 标准。完全使用符合国际标准的技术来实现。诸如unicode14.0,html5... 可以在几乎任何操作系统、浏览器上或者完全一致的体验。
2. 专业。尽力还原古籍原貌，不做妥协。
...
9. 无上士调御丈夫 中间的标点去掉
10. 校勘了大约数十万处原文错误的字词，相比cbeta变动超过500万处(数量是大概统计)
...
2. 2021-11-28 08:29:00 收到宏开法师500.00元
