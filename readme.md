...
6. terms.txt  佛教词汇大全，用来给藏经分词用的, 以便全文检索使用
7. w_normal.txt 制作的组合字表,相比原光盘的更加清晰, 用来清洗xml文档
8. w_norm2.txt 制作的未知组合字表, 需要填写

## 浏览器兼容性
1. firefox、opera、chrome、safari等主流浏览器都没有发现问题(ff出现了一个'载入样式表单出错： 分析 XSLT 样式表单失败。'的错误，暂时没找到原因)
2. 搜狗浏览器目前无法翻页
3. ie8仅仅是输入网址就直接卡死，Edge只能显示文字, 没有css效果; 这种不支持是在xslt层面不支持的，不能通过其他方式解决，ie看来是不能指望了，不要使用ie!
