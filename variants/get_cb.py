...
# Last Modified: 2018-10-06 20:08:01
...
sql = 'select uni, nun, nor, cipin, des, name, cipin  from cb2  where tag is true order by cipin desc'
cur.execute(sql)
data = cur.fetchall()
for i in data:
    uni = i[0] if i[0] else ''  # [18:]
    nun = i[1] if i[1] else ''
    nor = i[2] if i[2] else ''
    cipin = i[3]
    des = i[4]
    ...
    print(','.join((name, uni, nun, nor, des, cipin)))
    aa = aa + 1
    pass
